"""
Spectral-Locality Network v4
==============================
True synthesis incorporating ALL validated insights:

1. FREQUENCY MECHANISM IS DATA-DEPENDENT:
   - Tabular/1D latent: fixed integer-harmonic scalars, w_n = 2*pi*n / P,
     with P learnable scalar to adapt to data scale.
   - Image/multi-D latent: learnable multi-D frequency vectors
     (preserves distributed latent structure, as in V1).

2. DISTRIBUTED FOURIER REPRESENTATION:
   Output is [cos(proj), sin(proj)] (batch, 2*(N+1)) —
   never collapsed to scalar.

3. SMALL MLP HEAD: exactly ONE hidden layer.

4. Architecture depth: 3-4 layers total.

This combines SLP spectral fidelity with backprop scalability.
"""
...
# ---------------------------------------------------------------------------
# Fourier Layer: conditional 1D scalar vs multi-D vector frequencies
# ---------------------------------------------------------------------------
class FourierLayerV4(nn.Module):
    """
    If latent_dim == 1: fixed integer-harmonic scalars with learnable period.
    If latent_dim > 1: learnable multi-D frequency vectors (distributed).
    In both cases, output is [cos, sin] distributed features.
    """
    def __init__(self, latent_dim, num_modes, init_period=2.0):
        ...
        if latent_dim == 1:
            # Scalar harmonics: w_n = 2*pi*n / P, P learnable
            ...
            # Multi-D frequency vectors: each mode has a direction in latent space
            self.frequencies = nn.Parameter(torch.randn(num_modes + 1, latent_dim) * 0.1)
            self.period = None

    def forward(self, z):
        if self.latent_dim == 1:
            freqs = 2.0 * np.pi * self.harmonic_n / (self.period.abs() + 1e-6)
            proj = z.squeeze(-1).unsqueeze(1) * freqs.unsqueeze(0)  # (batch, num_modes+1)
        else:
            proj = torch.matmul(z, self.frequencies.T)  # (batch, num_modes+1)
...
class SpectralV4(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
        ...
        self.fourier = FourierLayerV4(latent_dim, num_modes, init_period)
    ...
    def forward(self, x):
        if self.input_type == "image":
            ...
            z = self.encoder(x)
        acts = self.fourier(z)
        return self.classifier(acts)

    def get_fourier_params(self):
        if self.fourier.latent_dim == 1:
            return {"period": self.fourier.period.item()}
        else:
            W = self.fourier.frequencies.detach().cpu().numpy()
            norms = np.linalg.norm(W, axis=1)
            return {"freq_norms": norms.tolist()}
...
def main():
    ...
    model = SpectralV4(
        ...
        init_period=args.init_period, head_hidden=args.head_hidden,
    ).to(device)

    print("="*60)
    print("SPECTRAL V4 MODEL")
    print("="*60)
    print(f"Latent dim: {args.latent_dim} | Fourier type: {'scalar-harmonic' if args.latent_dim==1 else 'multi-D vectors'}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if va > best_acc:
            best_acc = va
            torch.save({"state_dict": model.state_dict(), "fourier": model.get_fourier_params()},
                       f"best_v4_{args.dataset}.pt")
        if epoch <= 3 or epoch % 5 == 0 or epoch == args.epochs:
            print(f"Epoch {epoch:02d} | Train Acc: {ta:.4f} | Test Acc: {va:.4f}")
    print(f"Best Spectral V4 accuracy: {best_acc:.4f}\n")
    ...
    print(f"Fourier params: {model.get_fourier_params()}")
