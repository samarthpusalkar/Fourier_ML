"""
Spectral-Locality Network v5 (FINAL)
====================================
Generic unified architecture, no modality-specific branches.

Pipeline (all modalities):
  Raw Input -> Modality-Specific Encoder -> [Optional Spectral Mixer] ->
  Latent space (multi-D) -> Learnable 1D Projection ->
  Fixed Integer-Harmonic Fourier with Learnable Scale ->
  [Fourier_scalar + Latent_full] -> Single-Hidden-Layer Classifier

Key design choices:
  1. Fourier is always scalar, applied after 1D projection (preserves SLP spirit).
  2. Frequencies are fixed integer harmonics; only a global SCALE is learnable.
  3. Full latent is passed alongside Fourier scalar (no information collapse).
  4. Spectral Mixer is optional (disabled for tabular by setting window=1).
  5. Translation equivariance via parameter-shared spectral gains across patches.
"""
...
# ---------------------------------------------------------------------------
# Spectral Token Mixer (local or global, parameter-shared)
# ---------------------------------------------------------------------------
class SpectralTokenMixer(nn.Module):
    """
    Global FFT mixing by default.  For local mixing, use window > 1 (future).
    Currently supports full-sequence FFT with shared complex gains.
    """
    ...
    def forward(self, z):
        ...
        z_out = torch.fft.irfft(z_filtered, n=z.size(1), dim=-1).transpose(1, 2)
        return self.norm(z + z_out)


# ---------------------------------------------------------------------------
# Fixed-integer-harmonic Fourier with learnable 1D projection + scale
# ---------------------------------------------------------------------------
class GenericFourierHead(nn.Module):
    """
    Always operates on a 1D projection of the latent.
    Frequencies: integer harmonics w_n = 2*pi*n / scale, n=0..N.
    scale: learnable positive scalar.
    Output: fourier_scalar and the full latent for downstream use.
    """
    def __init__(self, latent_dim, num_modes, init_scale=2.0):
        ...
        # Learnable 1D projection direction (unit vector, but not enforced)
        self.proj_weight = nn.Parameter(torch.randn(latent_dim) * 0.1)
        self.proj_bias = nn.Parameter(torch.zeros(1))

        # Learnable global scale for frequencies
        self.scale = nn.Parameter(torch.tensor(init_scale, dtype=torch.float32))

        # Fixed harmonic indices (not parameters)
        self.register_buffer('harmonic_n', torch.arange(num_modes + 1, dtype=torch.float32))

    def forward(self, z):
        # z: (batch, latent_dim)
        # 1D projection
        z_1d = torch.matmul(z, self.proj_weight) + self.proj_bias  # (batch,)

        # Integer-harmonic frequencies with learnable scale
        freqs = 2.0 * np.pi * self.harmonic_n / (self.scale.abs() + 1e-6)  # (num_modes+1,)
        proj = z_1d.unsqueeze(1) * freqs.unsqueeze(0)  # (batch, num_modes+1)

        # SLP-style reconstruction (single scalar output)
        cos_terms = torch.cos(proj)
        sin_terms = torch.sin(proj)

        # Sum over modes to get Fourier scalar (like SLP output before rounding)
        # This is a FIXED weighting (A=1, B=1 per mode) for the scalar feature.
        # The LEARNABLE part is the classifier downstream.
        fourier_scalar = cos_terms.sum(dim=-1, keepdim=True) + sin_terms.sum(dim=-1, keepdim=True)
        fourier_scalar = fourier_scalar / (self.num_modes + 1)  # normalize amplitude

        return fourier_scalar, z  # return both scalar and full latent


# ---------------------------------------------------------------------------
# Unified model (generic, no modality branches)
# ---------------------------------------------------------------------------
class SpectralV5(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
                 ...
                 num_classes=10, head_hidden=64, init_scale=2.0):
        super().__init__()
        self.input_type = input_type

        # --- Encoder (modality-specific ONLY here; everything else is generic) ---
        if input_type == "image":
            ph, pw = patch_size
        ...
        # --- Generic components (same for ALL modalities) ---
    ...
    def forward(self, x):
        ...
        features = torch.cat([fourier_scalar, z_full], dim=-1)  # (B, 1 + latent_dim)
    ...
    def get_fourier_info(self):
        return {
            ...
            "proj_bias": self.fourier_head.proj_bias.item(),
        }
...
def main():
    ...
    print(f"Best Spectral V5 accuracy: {best_acc:.4f}\n")
    ...
    print(f"Final Fourier info: {model.get_fourier_info()}")
