"""
Spectral Activation Sweep: Test Different Non-Linearities
=========================================================

Compare activations placed between spectral mixer layers.
Each mixer: FFT → gain → IFFT → [activation] → norm+residual

Activations tested:
  - none       (baseline, linear)
  - gelu       (smooth, no sharp corners, spatial domain)
  - swish      (self-gated smooth)
  - softplus   (smooth ReLU-like)
  - elu        (exponential linear, negative saturation)
  - threshold  (frequency-domain tanh(magnitude/tau))
"""
...
class NDSpectralMixerAct(nn.Module):
    """
    Spectral mixer with optional activation between layers.
    FFT → gain → IFFT → [activation] → norm + residual
    """
    def __init__(self, latent_dim, spatial_shape, activation="none"):
        super().__init__()
        self.latent_dim = latent_dim
        h, w = spatial_shape
...
class SpectralActivation(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
                 ...
                 num_classes=10, head_hidden=128, init_scale=2.0, activation="none"):
        super().__init__()
        self.input_type = input_type
        self.latent_dim = latent_dim

        if input_type == "image":
            self.embed = NDSpatialPatchEmbed(patch_size, in_ch, latent_dim)
            ph, pw = patch_size
            spatial_shape = (img_size[0] // ph, img_size[1] // pw)
            self.mixers = nn.ModuleList([
                NDSpectralMixerAct(latent_dim, spatial_shape, activation)
...
def run_experiment(args, activation):
    ...
    if args.dataset == "mnist":
        ...
        input_type = "image"; img_size = (28, 28); in_ch = 1; num_classes = 10
        ...
        input_type = "image"; img_size = (32, 32); in_ch = 3; num_classes = 10
        ...
        x_tr = torch.from_numpy(x_train); x_te = torch.from_numpy(x_test)
        y_tr = torch.from_numpy(y_train.astype(np.int64)); y_te = torch.from_numpy(y_test.astype(np.int64))
        input_type = "flat"; img_size = None; in_ch = None; num_classes = 3
    ...
    model = SpectralActivation(
        ...
        head_hidden=args.head_hidden, init_scale=args.init_scale, activation=activation,
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if va > best_acc:
            best_acc = va
            best_epoch = epoch
    return best_acc, best_epoch, count_params(model)
...
def main():
    ...
    activations = ["none", "relu", "gelu", "swish", "softplus", "elu"]

    print(f"Device: {'cuda' if torch.cuda.is_available() else 'cpu'} | Dataset: {args.dataset}")
    print(f"Sweeping activations: {activations}")
    print(f"Patch: {tuple(args.patch_size)} | Epochs: {args.epochs}\n")
    print("="*60)
    print(f"{'Activation':<15} {'Best Acc':<12} {'Epoch':<8} {'Params':<10}")
    print("-"*60)

    for act in activations:
        try:
            best_acc, best_epoch, params = run_experiment(args, act)
            print(f"{act:<15} {best_acc:<12.4f} {best_epoch:<8} {params:<10}")
        except Exception as e:
            print(f"{act:<15} FAILED: {e}")
