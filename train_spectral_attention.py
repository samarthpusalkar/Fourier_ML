"""
Spectral-Attention Model: Learnable Spatial Focus
=================================================

After 2D FFT spatial mixing, each patch position has a feature vector.
Instead of blind global mean pooling, use learnable spatial attention:
  - Compute per-position relevance score (1x1 conv → scalar)
  - Softmax over spatial grid
  - Weighted sum of features

This gives the model "where to look" capability without convolutions.
"""
...
class SpatialAttentionPool(nn.Module):
    """
    Learnable spatial attention: each spatial position gets a score,
    softmax over positions, weighted sum of features.
    """
    def __init__(self, latent_dim):
        super().__init__()
        self.score_proj = nn.Linear(latent_dim, 1)

    def forward(self, z):
        # z: (B, latent_dim, H, W)
        B, C, H, W = z.shape
        z_perm = z.permute(0, 2, 3, 1).contiguous()  # (B, H, W, C)
        scores = self.score_proj(z_perm).squeeze(-1)  # (B, H, W)
        attn = F.softmax(scores.view(B, -1), dim=-1).view(B, H, W)  # (B, H, W)
        # Weighted sum: (B, C, H, W) * (B, 1, H, W) -> sum over H,W
        weighted = z * attn.unsqueeze(1)
        pooled = weighted.sum(dim=[-2, -1])  # (B, C)
        return pooled, attn
...
class SpectralAttention(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
        ...
        self.attn_pool = SpatialAttentionPool(latent_dim)
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True,
                        choices=["mnist", "fashion", "cifar10", "iris", "wine", "breast_cancer", "digits"])
    ...
    if args.dataset == "mnist":
        ...
        x_tr, y_tr, x_te, y_te = _load_wine()
        input_type = "flat"; img_size = None; in_ch = None; num_classes = 3
    elif args.dataset == "breast_cancer":
        x_tr, y_tr, x_te, y_te = _load_breast_cancer()
        input_type = "flat"; img_size = None; in_ch = None; num_classes = 2
    elif args.dataset == "digits":
        x_tr, y_tr, x_te, y_te = _load_digits()
        input_type = "image"; img_size = (8, 8); in_ch = 1; num_classes = 10
    ...
    print(f"Architecture: Spatial Attention + 2D FFT Mixer\n")
    print("="*60)
    print("SPECTRAL ATTENTION MODEL")
    print("="*60)
    ...
    print(f"Best Spectral Attention accuracy: {best_acc:.4f}\n")
