"""
Spectral Patch Filter: Per-Patch FFT Preprocessing
=====================================================

Core idea: Before encoding a patch with Linear(48->latent_dim),
filter the patch in frequency domain to suppress noise/background clutter.

Each patch is a tiny 2D signal (e.g., 4x4).
- Compute 2D FFT of each patch
- Apply learnable per-channel complex gains (low-pass characteristic)
- Inverse FFT back to spatial
- Encode filtered patch

This separates signal (coarse structure) from noise (fine texture, background)
at the SOURCE, before any mixing or global pooling.
"""
...
# ---------------------------------------------------------------------------
# Patch FFT Filter Embed
# ---------------------------------------------------------------------------
class PatchFFTEmbed(nn.Module):
    """
    For each spatial patch, perform 2D FFT, apply learnable per-channel complex gains,
    then IFFT back to spatial. Encode filtered patch with Linear.

    Output: (B, latent_dim, H', W')
    """
    def __init__(self, patch_size, in_ch, latent_dim, init_gain=0.5):
        super().__init__()
        ...
        # Learnable frequency-domain gains per patch dimension
        # For 2D FFT of patch: freq_shape = (ph, pw//2 + 1)
        self.freq_shape = (ph, pw // 2 + 1)
        self.gain_real = nn.Parameter(torch.ones(in_ch, *self.freq_shape) * init_gain)
        self.gain_imag = nn.Parameter(torch.zeros(in_ch, *self.freq_shape))
    ...
    def forward(self, x):
        ...
        # Reshape for per-patch FFT: (B*H'*W', C, ph, pw)
        Bp, Cp, Hp, Wp, Ph, Pw = x.shape
        patches = x.permute(0, 2, 3, 1, 4, 5).contiguous().view(-1, Cp, Ph, Pw)

        # 2D FFT per patch
        patches_fft = torch.fft.rfft2(patches, dim=(-2, -1))  # (N, C, ph, pw//2+1)
        gain = torch.view_as_complex(torch.stack([self.gain_real, self.gain_imag], dim=-1))
        patches_filtered = patches_fft * gain.unsqueeze(0)  # broadcast N
        patches_filtered = torch.fft.irfft2(patches_filtered, s=(ph, pw), dim=(-2, -1))

        # Reshape back to spatial grid
        patches_filtered = patches_filtered.view(Bp, Hp, Wp, Cp, ph, pw)
        patches_filtered = patches_filtered.permute(0, 3, 1, 2, 4, 5).contiguous()
        patches_filtered = patches_filtered.view(Bp, Cp, Hp, Wp, Ph * Pw)
        patches_filtered = patches_filtered.permute(0, 2, 3, 1, 4).contiguous()
        patches_filtered = patches_filtered.view(Bp * Hp * Wp, -1)

        z = self.encoder(patches_filtered)
        z = z.view(Bp, Hp, Wp, self.latent_dim)
        z = z.permute(0, 3, 1, 2).contiguous()
        return z


# ---------------------------------------------------------------------------
# N-Dimensional Spectral Mixer (same as N-D model)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Spectral FFT-Filter Model
# ---------------------------------------------------------------------------
class SpectralFFTFilter(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
        ...
        if input_type == "image":
            self.embed = PatchFFTEmbed(patch_size, in_ch, latent_dim)
            ph, pw = patch_size
...
def main():
    ...
    print(f"Architecture: Per-Patch FFT Filter -> N-D Spatial FFT Mixer\n")
    ...
    print("="*60)
    print("SPECTRAL FFT-FILTER MODEL")
    print("="*60)
    ...
    print(f"Best Spectral FFT-Filter accuracy: {best_acc:.4f}\n")
