"""
Spectral v6 (No Cosine LR): Global 2D FFT + Learnable Square Wave
Constant LR ablation to test if epoch-5 slowdown is LR-related.
"""
import torch
...
class CompleteFourierHead(nn.Module):
    def __init__(self, latent_dim, num_modes=64, init_scale=2.0, spacing_mode="uniform"):
        super().__init__()
...
class GlobalSpectralModel(nn.Module):
    def __init__(self, img_size, in_ch, latent_dim=16, num_modes=64,
        ...
        self.fourier_head = CompleteFourierHead(latent_dim, num_modes, init_scale, spacing_mode)
...
def main():
    ...
    print(f"NO COSINE LR — constant {args.lr} across all epochs")
    print(f"Architecture: Global 2D FFT + 1x1 conv + {args.activation} activation\n")
    print("="*60)
    print("GLOBAL SPECTRAL MODEL (NO COSINE LR)")
    print("="*60)
