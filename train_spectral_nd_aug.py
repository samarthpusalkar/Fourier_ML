"""
N-D Spectral Model with Data Augmentation
=========================================

Random crop + horizontal flip for image datasets to teach
position/scale invariance.

Augmentation is the missing ingredient for CIFAR-10. Objects
appear at different positions and scales. Without augmentation,
the model overfits to absolute pixel positions.
"""
...
def _download_fashion(data_dir):
    ...
    for key, fname in FASHION_FILES.items():
        ...
        if not os.path.exists(fpath):
            ...
            urllib.request.uuretrieve(FASHION_URL + fname, fpath)
...
class RandomCropFlip:
    """
    Random crop with padding + random horizontal flip.
    For CIFAR-10: pad 4, crop 32x32, flip p=0.5
    For MNIST/Fashion: pad 2, crop 28x28, flip p=0.5 (asymmetric digits hurt)
    """
    def __init__(self, img_size, pad=4, flip_p=0.5):
    ...
    def __call__(self, x):
        ...
        x = x[:, top:top + self.img_size[0], left:left + self.img_size[1]]
        if np.random.rand() < self.flip_p:
            x = torch.flip(x, dims=[-1])
        return x


def augment_batch(batch, img_size, pad=4, flip_p=0.5):
    """Apply augmentation to a batch of images."""
    B, C, H, W = batch.shape
    if pad > 0:
        batch = F.pad(batch, [pad] * 4, mode='reflect')
    _, _, hp, wp = batch.shape
    tops = torch.randint(0, 2 * pad + 1, (B,))
    lefts = torch.randint(0, 2 * pad + 1, (B,))
    cropped = []
    for i in range(B):
        top, left = tops[i].item(), lefts[i].item()
        crop = batch[i:i+1, :, top:top + img_size[0], left:left + img_size[1]]
        cropped.append(crop)
    batch = torch.cat(cropped, dim=0)
    if flip_p > 0:
        flips = torch.rand(B) < flip_p
        for i in range(B):
            if flips[i]:
...
# ---------------------------------------------------------------------------
# N-D Spectral Model with Augmentation
# ---------------------------------------------------------------------------
class NDSpectralModelAug(nn.Module):
...
def train_epoch(model, loader, optimizer, criterion, device, augment_fn=None):
    model.train()
...
def main():
    ...
    parser.add_argument("--dataset", type=str, required=True, choices=["mnist", "fashion", "cifar10", "iris"])
    ...
    parser.add_argument("--augment_pad", type=int, default=4, help="Padding for random crop")
    parser.add_argument("--augment_flip", type=float, default=0.5, help="Horizontal flip probability")
    ...
    if input_type == "image":
        augment_fn = lambda x: augment_batch(x, img_size, pad=args.augment_pad, flip_p=args.augment_flip)
    else:
        augment_fn = None

    print("="*60)
    print("N-D SPECTRAL MODEL WITH AUGMENTATION")
    print("="*60)
    print(f"Dataset: {args.dataset} | Augment pad: {args.augment_pad} | Flip p: {args.augment_flip}")
    print(f"Latent dim: {args.latent_dim} | FFT: 2D over patch grid")
    ...
    for epoch in range(1, args.epochs + 1):
        tl, ta = train_epoch(model, train_loader, optimizer, criterion, device, augment_fn=augment_fn)
    ...
    print(f"Best Augmented N-D accuracy: {best_acc:.4f}\n")
    ...
    if args.run_baseline:
        ...
        best_b = 0.0
        for epoch in range(1, args.epochs + 1):
            tl, ta = train_epoch(baseline, train_loader, opt_b, criterion, device, augment_fn=augment_fn)
