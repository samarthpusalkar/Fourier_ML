"""
Spectral Threshold: Frequency-Domain Non-Linearity
====================================================

Complex Magnitude Soft Thresholding inside each spectral mixer:
  FFT → gain → tanh(magnitude / tau) → IFFT → norm

tau is learnable per frequency bin. This is spectral-native
non-linearity with no Gibbs artifacts.
"""
...
# ---------------------------------------------------------------------------
# Complex Magnitude Soft Thresholding Spectral Mixer
# ---------------------------------------------------------------------------
class SpectralThresholdMixer(nn.Module):
    """
    FFT → apply learnable complex gain → tanh(magnitude/tau) soft-threshold
    → IFFT → norm + residual
    """
    def __init__(self, latent_dim, spatial_shape):
        ...
        # Learnable per-frequency threshold tau
        self.log_tau = nn.Parameter(torch.ones(latent_dim, *self.freq_shape) * -2.0)
    ...
    def forward(self, z):
        ...
        z_gain = z_fft * gain.unsqueeze(0)

        # Soft thresholding in frequency domain
        magnitude = z_gain.abs() + 1e-8
        tau = torch.exp(self.log_tau).unsqueeze(0)  # positive, learnable
        soft_gate = torch.tanh(magnitude / tau)
        z_thresholded = z_gain * soft_gate

        original_shape = z.shape[-2:]
        z_out = torch.fft.irfft2(z_thresholded, s=original_shape, dim=(-2, -1))
...
class SpectralThreshold(nn.Module):
    def __init__(self, input_type, img_size=None, patch_size=None, in_ch=None,
        ...
        if input_type == "image":
            ...
            self.mixers = nn.ModuleList([
                SpectralThresholdMixer(latent_dim, spatial_shape)
...
def main():
    ...
    print(f"Architecture: Complex Magnitude Soft Thresholding in Frequency Domain\n")
    print("="*60)
    print("SPECTRAL THRESHOLD MODEL")
    print("="*60)
    print(f"Latent dim: {args.latent_dim} | Patch: {tuple(args.patch_size)}")
    ...
    print(f"Best Spectral Threshold accuracy: {best_acc:.4f}\n")
