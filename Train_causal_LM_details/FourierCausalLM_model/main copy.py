...
class CausalContinuousFourierMixer1D(nn.Module):
    def __init__(self, channels, num_modes=128, num_heads=12):
        ...
        # ---------------------------------------------------------------------
        # freq_bands = torch.stack([torch.tensor(math.exp(math.log(0.0001)+i*(0-math.log(0.0001))/num_modes)) for i in range(num_modes)] + [torch.tensor(j) for j in range(1, num_modes + 1)])
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        hub_model_id="CodeIsAbstract/random_model_test",              
