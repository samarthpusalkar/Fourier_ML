...
class CausalContinuousFourierMixer1D(nn.Module):
    ...
    def forward(self, x):
        ...
        # During training (seq_len=512), steps are exactly 1/32. 
        ...
        # OPTIMIZATION: Use implicit broadcasting instead of .expand().contiguous()
        P_cos = U.unsqueeze(1) * W_cos.unsqueeze(0) + V.unsqueeze(1) * W_sin.unsqueeze(0) # (seq_len, num_heads, num_modes)
        P_sin = V.unsqueeze(1) * W_cos.unsqueeze(0) - U.unsqueeze(1) * W_sin.unsqueeze(0) # (seq_len, num_heads, num_modes) 
        ...
        # 4. Route Values (v1) using einsum for highly efficient broadcasting
        v1_heads = v1.view(B, seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        
        # OPTIMIZATION: Use einsum to perform batched matmul without explicitly expanding K_matrix over the batch dimension
        v1_token_mixed = torch.einsum('h s l, b h l d -> b h s d', K_matrix, v1_heads)
        
        # 5. Flatten back to standard format safely
        # Using .reshape() handles the contiguity requirements automatically
...
def get_datasets(seq_length=512):
    ...
    # Using GPT-NeoX tokenizer (50k vocab, Byte-Level BPE) instead of BERT (WordPiece)
    # This is a standard, highly efficient tokenizer for modern Causal LMs.
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-neox-20b", model_max_length=int(1e9))
    
    # Ensure it has a pad token for the collator
...
def main():
    ...
    training_args = TrainingArguments(
        ...
        hub_model_id="CodeIsAbstract/OptimizedFourierCausalLM",              
