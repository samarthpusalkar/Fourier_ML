...
BASE_PATH_STORAGE = '/mnt/gpt-train'
...
class HybridFourierLM(nn.Module):
    ...
    def forward(self, input_ids, labels=None, **kwargs):
        ...
        # # --- NEW CHECKPOINTING LOOP ---
        # for mixer in self.mixers: 
        #     if self.gradient_checkpointing and self.training:
        #         # Discards activations and recalculates during backward pass
        #         z = checkpoint(mixer, z, use_reentrant=False)
        #     else:
        #         z = mixer(z)
        # # ------------------------------
        for mixer in self.mixers: 
...
def main():
    ...
    parser.add_argument("--resume_from_hub_id", type=str, default='CodeIsAbstract/Hybrid_test_model_kaggle', help="HuggingFace repo ID to pull last-checkpoint from")
    parser.add_argument("--seq_len", type=int, default=512)
    parser.add_argument("--learning_rate", type=float, default=1e-6) # LOWERED FROM 2e-3
    ...
    if flags.resume_from_hub_id:
        ...
        dir_downloaded = local_dir + "last-checkpoint"
        flags.load_weights = OUTPUT_PATH + "/checkpoint-1000/model.safetensors"
        flags.load_weights = dir_downloaded + "/model.safetensors"
        print(f"Resuming training from downloaded checkpoint: {flags.load_weights}")
    ...
    training_args = TrainingArguments(
        ...
        torch_compile=False if torch.cuda.is_available() else False,              
        bf16=use_bf16,                   
        ...
        hub_model_id="CodeIsAbstract/Hybrid_test_model_continued_modal",
        
        report_to="none",
        dataloader_num_workers=3 if torch.cuda.is_available() else 0,
